...
class IMUInterface
{
    protected:
        ...
        geometry_msgs__msg__Vector3 accel_cal_;

        geometry_msgs__msg__Vector3 applyAccelTweak(
            geometry_msgs__msg__Vector3 accel)
        {
            imu_msg_.linear_acceleration = accel;
#ifdef IMU_TWEAK
            IMU_TWEAK
#endif
            return imu_msg_.linear_acceleration;
        }
        ...
        void calibrateAccelerometer()
        {
            ...
            {
                geometry_msgs__msg__Vector3 accel =
                    applyAccelTweak(readAccelerometer());

                accel_cal_.x += accel.x;
                accel_cal_.y += accel.y;
                accel_cal_.z += accel.z;

                delay(50);
            }

            accel_cal_.x = accel_cal_.x / (float)sample_size_;
            accel_cal_.y = accel_cal_.y / (float)sample_size_;
            accel_cal_.z = accel_cal_.z / (float)sample_size_ - 9.8;
        }
    ...
    public:
        ...
        sensor_msgs__msg__Imu getData()
        {
            ...
            imu_msg_.linear_acceleration =
                applyAccelTweak(readAccelerometer());
            imu_msg_.linear_acceleration.x -= accel_cal_.x;
            imu_msg_.linear_acceleration.y -= accel_cal_.y;
            imu_msg_.linear_acceleration.z -= accel_cal_.z;
