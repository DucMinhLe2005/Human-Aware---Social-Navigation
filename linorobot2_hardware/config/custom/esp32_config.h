...
// accelerometer scale correction: measured at-rest magnitude was 10.68 m/s^2
// instead of 9.81 m/s^2, so scale all axes by 9.81/10.68 to correct
#define IMU_TWEAK { \
    imu_msg_.linear_acceleration.x *= 0.918230; \
    imu_msg_.linear_acceleration.y *= 0.918230; \
    imu_msg_.linear_acceleration.z *= 0.918230; \
}
// #define MAG_TWEAK {}
...
#define MOTOR_POWER_MEASURED_VOLTAGE 11.7     // current voltage reading of the power connected to the motor (used for calibration)
#define COUNTS_PER_REV1 1700                // wheel1 encoder's no of ticks per rev
#define COUNTS_PER_REV2 1700                 // wheel2 encoder's no of ticks per rev
#define COUNTS_PER_REV3 1                 // wheel3 encoder's no of ticks per rev
#define COUNTS_PER_REV4 1                 // wheel4 encoder's no of ticks per rev
#define WHEEL_DIAMETER 0.08               // wheel's diameter in meters
#define LR_WHEELS_DISTANCE 0.3           // distance between left and right wheels (measured)
